...
for n_items_c1 in range(1, ceil(len(weights)/3)):
    for items_c1 in combinations(weights, n_items_c1):
        if np.sum(items_c1) == compartment_weight:
            ...
            for n_items_c2 in range(n_items_c1, ceil(len(remaining_weights)/2)):
                for items_c2 in combinations(remaining_weights, n_items_c2):
                    ...
                    if np.sum(items_c2) == compartment_weight:
                        items_c3 = [x for x in weights if x not in used_weights]
                        if len(items_c3) < n_items_c1:
                            continue
                        else:
                            qe = np.prod(items_c1)
                            if qe < qe_min:
